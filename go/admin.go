...
func (h *Handler) adminListMaster(c *fiber.Ctx) error {
	...
	if err := h.getAdminDB().Select(&masterVersions, "SELECT * FROM version_masters"); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}

	items := make([]*ItemMaster, 0)
	if err := h.getAdminDB().Select(&items, "SELECT * FROM item_masters"); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}

	gachas := make([]*GachaMaster, 0)
	if err := h.getAdminDB().Select(&gachas, "SELECT * FROM gacha_masters"); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}

	gachaItems := make([]*GachaItemMaster, 0)
	if err := h.getAdminDB().Select(&gachaItems, "SELECT * FROM gacha_item_masters"); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}

	presentAlls := make([]*PresentAllMaster, 0)
	if err := h.getAdminDB().Select(&presentAlls, "SELECT * FROM present_all_masters"); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)

	}

	loginBonuses := make([]*LoginBonusMaster, 0)
	if err := h.getAdminDB().Select(&loginBonuses, "SELECT * FROM login_bonus_masters"); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)

	}

	loginBonusRewards := make([]*LoginBonusRewardMaster, 0)
	if err := h.getAdminDB().Select(&loginBonusRewards, "SELECT * FROM login_bonus_reward_masters"); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}
...
func (h *Handler) adminUpdateMaster(c *fiber.Ctx) error {
	...
	if versionMasterRecs != nil {
		...
		log.Debug().Msg("Skip Update Master: versionMaster")
	}
	...
	if itemMasterRecs != nil {
		...
		log.Debug().Msg("Skip Update Master: itemMaster")
	}
	...
	if gachaRecs != nil {
		...
		log.Debug().Msg("Skip Update Master: gachaMaster")
	}
	...
	if gachaItemRecs != nil {
		...
		log.Debug().Msg("Skip Update Master: gachaItemMaster")
	}
	...
	if presentAllRecs != nil {
		...
		log.Debug().Msg("Skip Update Master: presentAllMaster")
	}
	...
	if loginBonusRecs != nil {
		...
		log.Debug().Msg("Skip Update Master: loginBonusMaster")
	}
	...
	if loginBonusRewardRecs != nil {
		...
		log.Debug().Msg("Skip Update Master: loginBonusRewardMaster")
	}
...
func (h *Handler) adminBanUser(c *fiber.Ctx) error {
	...
	if _, err = h.getAdminDB().Exec(query, banID, userID, requestAt, requestAt, requestAt); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}
	if err := h.UB.Load(h); err != nil {
		return errorResponse(c, http.StatusInternalServerError, err)
	}
